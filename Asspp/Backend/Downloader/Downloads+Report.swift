...
@MainActor
extension Downloads {
    func edit(id: Request.ID, _ callback: @escaping (inout Request) -> Void) async {
        guard let index = requests.firstIndex(where: { $0.id == id }) else {
            logger.warning("[!] request id not found for alteration: \(id)")
            return
        }
        ...
        guard req.hashValue != hashValue else {
            logger.debug("[=] request unchanged after alteration: \(id)")
            return
        }
        requests[index] = req
    }
    ...
    func report(speed: String, reqId: Request.ID) async {
        await edit(id: reqId) { $0.runtime.speed = speed }
    }
}
